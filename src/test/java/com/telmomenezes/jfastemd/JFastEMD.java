...
public class JFastEMD {
     ...
     * <p>
     * To get the same results as Rubner's code you should set extra_mass_penalty to 0, and divide by the minimum of the
     * sum of the two signature's weights. However, I suggest not to do this as you lose the metric property and more
     * importantly, in my experience the performance is better with emd_hat. for more on the difference between emd and
     * emd_hat, see the paper: A Linear Time Histogram Metric for Improved SIFT Matching Ofir Pele, Michael Werman ECCV
     * 2008
     * <p>
     * To get shorter running time, set the ground distance function to be a thresholded distance. For example: min(L2,
     * T). Where T is some threshold. Note that the running time is shorter with smaller T values. Note also that
     * thresholding the distance will probably increase accuracy. Finally, a thresholded metric is also a metric. See
     * paper: Fast and Robust Earth Mover's Distances Ofir Pele, Michael Werman ICCV 2009
     * <p>
    ...
    static public long emdHatImplLongLongInt(Vector<Long> Pc, Vector<Long> Qc, Vector<Vector<Long>> C, long extraMassPenalty) {
